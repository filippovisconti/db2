...
Considerato che Postgres utilizza il **Multi-Version Concurrency Control**, a differenza dei sistemi basati strettamente sui lock (come il 2PL teorico), in Postgres le operazioni di lettura non bloccano mai le scritture e viceversa. Ogni transazione interroga uno snapshot del database. Questo approccio ottimistico fa sì che l'implementazione reale dei livelli di isolamento in Postgres si discosti parzialmente dallo standard ANSI SQL. Nello specifico:

* Il livello `READ UNCOMMITTED` non è realmente supportato ed è trattato internamente come `READ COMMITTED`.
...
* Il livello `SERIALIZABLE` è gestito tramite _Serializable Snapshot Isolation (SSI)_, che garantisce l'isolamento risolvendo i cicli di dipendenza tramite l'aborto di una delle transazioni coinvolte.

Questi comportamenti sono stati dimostrati empiricamente attraverso 6 scenari di test.

# Esperimenti e Analisi dei Risultati

Di seguito vengono illustrati gli scenari eseguiti tramite il software Python e l'analisi dei log generati, confrontando il comportamento teorico con le reali esecuzioni sul DBMS.

## Scenario 1: Lost Update 
Questo scenario prevede due transazioni concorrenti che leggono un dato (il conteggio dell'impiegato) e tentano di incrementarlo.

Eseguendo il test in livello `READ COMMITTED`, entrambe le transazioni leggono il medesimo valore iniziale e procedono all'aggiornamento, portando alla perdita di una delle due modifiche (Lost Update).
...
In questo test, una transazione (T1) legge lo stesso record due volte a distanza di tempo, mentre una seconda transazione (T2) lo modifica ed effettua il commit nell'intervallo tra le due letture.

Nel livello di default `READ COMMITTED`, i risultati mostrano che T1 legge due valori differenti. Lo snapshot viene infatti aggiornato all'inizio di ogni singola query.
...
Impostando T1 in `REPEATABLE READ`, la seconda query continua a restituire il valore originale (quello letto la prima volta), ignorando il commit effettuato da T2. Questo conferma che lo snapshot viene mantenuto coerente e inalterato per l'intera durata della transazione T1.
...
In questo test, la transazione T1 esegue un'operazione di aggregazione (conteggio degli impiegati con valore > 0). Nel frattempo, T2 inserisce un nuovo impiegato che soddisfa tale condizione.
...
In teoria, il livello `REPEATABLE READ` tollera le letture fantasma. Tuttavia, i log dell'esperimento dimostrano che in Postgres questa anomalia non si verifica. Grazie al modello MVCC, T1 interroga uno snapshot creato all'inizio della transazione che rimane immutabile, isolandola dai nuovi inserimenti confermati da T2. Di fatto, in Postgres, il livello `REPEATABLE READ` offre una protezione superiore rispetto allo standard.
...
Questo scenario riproduce esattamente l'esercizio d'esame presentato a lezione. Due transazioni concorrenti leggono il numero totale degli impiegati per poi inserire un nuovo record basato su tale conteggio.

In REPEATABLE READ, entrambe le transazioni leggono la stessa base di partenza (  `count=2` ). Ognuna procede con il proprio aggiornamento e il proprio commit. Nessuna delle due tocca le righe modificate dall'altra, quindi il DBMS non rileva conflitti diretti a livello di record e permette a entrambe di concludere con successo, lasciando passare l'anomalia.
...
```
Impostando l'isolamento a livello `SERIALIZABLE`, il sistema garantisce un'esecuzione serializzabile, ma con un approccio peculiare. Anziché utilizzare lock bloccanti fin dalla fase di lettura, Postgres applica il _Serializable Snapshot Isolation (SSI)_. Monitora le dipendenze in lettura e scrittura e, al rilevamento di un ciclo di dipendenze (entrambe hanno preso decisioni di scrittura basandosi su dati che l'altra stava modificando) che violerebbe la serializzabilità, lascia procedere la prima transazione ed esegue l'aborto forzato della seconda al momento del commit (restituendo un'eccezione di _Serialization Failure_, Errore 40001).
...
Questo scenario è stato introdotto specificamente per dimostrare lo scostamento tra lo standard SQL e l'implementazione del motore Postgres. T1 effettua un aggiornamento di un record senza fare commit (procedendo poi con un rollback), mentre T2 tenta di leggere il record modificato.
...
# Conclusioni
Gli esperimenti condotti hanno permesso di verificare empiricamente i concetti di controllo della concorrenza illustrati a lezione. La sperimentazione su PostgreSQL ha evidenziato come l'utilizzo del modello MVCC allontani parzialmente il DBMS dalle definizioni classiche basate sul Two-Phase Locking (2PL). L'approccio ottimistico di Postgres garantisce l'assenza di lock in lettura e una prevenzione stringente delle anomalie (come i Phantom Read già in `REPEATABLE READ`), scaricando però a livello applicativo la necessità di intercettare e gestire tramite retry le transazioni interrotte forzatamente (errori 40001 e 40P01) per garantire la corretta serializzabilità.
